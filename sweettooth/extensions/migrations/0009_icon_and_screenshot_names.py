...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        try:
            os.makedirs(os.path.join(settings.MEDIA_ROOT, 'screenshots'))
        except os.error:
            pass

        try:
            os.makedirs(os.path.join(settings.MEDIA_ROOT, 'icons'))
        except os.error:
            pass

        for ext in orm.Extension.objects.all():
            if ext.screenshot:
                new_screenshot = os.path.join("screenshots", "screenshot_%d.png" % (ext.pk,))
                os.rename(ext.screenshot.url, os.path.join(settings.MEDIA_ROOT, new_screenshot))
                ext.screenshot = new_screenshot

            new_icon = os.path.join("icons", "icon_%d.png" % (ext.pk,))
            if ext.icon.path == orm.Extension._meta.get_field_by_name('icon')[0].default:
                # Don't try to move the default
                continue
            shutil.copy(ext.icon.path, os.path.join(settings.MEDIA_ROOT, new_icon))
            ext.icon = new_icon
            ext.save()
